...
class ProductItem(BaseModel):
    ...
    suggested_pack_multiple: int = Field(1, description="Pack multiple suggested by heuristic analysis or override (for tablet guidance)")
    enforced_pack_multiple: int = Field(1, description="Business-approved pack multiple enforced on wholesale orders (1 unless approved explicit rule exists)")
    pack_multiple: int = Field(1, description="Active enforced pack multiple for wholesale validation (backward-compatible)")
    pack_source: Optional[str] = Field("fallback", description="Resolution source: override, heuristic, or fallback")
    approval_status: str = Field("DEFAULT", description="Approval status: APPROVED, UNVERIFIED_HEURISTIC, DEFAULT, PENDING_REVIEW, DISABLED")
